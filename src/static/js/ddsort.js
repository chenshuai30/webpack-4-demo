	...
	$.fn.DDSort = function( options ){
		...
		return this.each(function(){
			...
			that.on( 'mousedown.DDSort', settings.target, function( e ){
				...
				if(tagName == 'span'){
					if(className !== 'quesitemtitle'){
						return;
					}
				}
				if(tagName == 'i'){
					if(className.indexOf('aia-icons') > 0){
						// console.log('点击i');
						return;
					}
				}
				if(className == 'singleChioceName' || className == 'fr radiobtn' || className == 'el-button el-button--default el-button--mini'){return;}
				...
				$doc.on( 'mousemove.DDSort', function( e ){
					...
					let curSpan = $this.find('span.aia-sort');
					let prevSpan = prev.find('span.aia-sort');
					let nextSpan = next.find('span.aia-sort');
					let curIndex = parseInt(curSpan.text().split('.')[0]);
					let prevIndex = parseInt(prevSpan.text().split('.')[0]);
					let nextIndex = parseInt(nextSpan.text().split('.')[0]);					
					...
					if( prev.length && top < prev.offset().top + prev.outerHeight()/2 ){							
						...
						curIndex -=1;
						prevIndex +=1;		
						curSpan.text(curIndex +'.');
						prevSpan.text(prevIndex +'.');	
				...
				.on( 'mouseup.DDSort', function(){
					...
					if( !hasClone ){
						...
						let arr = $('#mainlist .quesItem');
						let result = [];
				  
						for(let i = 0;i<arr.length;i++){
						  let el = $(arr[i]);
						  let key = parseInt(el.attr('index'));
						  result.push(settings.own.quesList[key]);
						}
						settings.own.quesList = [];
						settings.own.$nextTick(()=>{
						  result.map(value=>{
							settings.own.quesList.push(value);
						  })
						})
					}
				});
